...
WATCHLIST_FILE = "watchlist.csv" # <-- NOUVEAU V5
...
def render_watchlist():
    ...
    for i, asset in enumerate(watchlist):
        with cols[i]:
            if st.button(f"⭐ {asset}", key=f"wl_{asset}", use_container_width=True):
                st.session_state.selected_asset_from_watchlist = asset
                st.rerun()
    ...
    with st.expander("➕ Gérer ma Watchlist"):
        ...
        if st.button("💾 Enregistrer"):
            save_watchlist(st.session_state.user_email, selected)
...
def get_market_news():
    return ["📰 Bitcoin reste au-dessus de son support principal.","📰 Nvidia publie des résultats solides.","📰 Les marchés attendent la décision de la Fed."]
...
def assistant_gpt4(question, contexte):
    ...
    try:
        ...
        response = model.generate_content(f"Tu es PrediTrade AI expert trading. Réponds en français. {question} Contexte: {contexte}")
...
# ============== 2. SIDEBAR ==============
st.sidebar.title("🚀 PrediTrade AI V5.0")
st.sidebar.write(f"📧 {st.session_state.user_email}")
if st.session_state.is_premium:
    st.sidebar.markdown('<span class="badge-premium">⭐ PREMIUM</span>', unsafe_allow_html=True)
else:
...
if menu == "📊 Tableau de bord":
    st.header("📊 Tableau de bord V5")

    render_watchlist() # <-- APPEL WATCHLIST
    st.divider()

    st.subheader("📰 Actualités du marché")
    for news in get_market_news(): st.info(news)
    ...
    c4.metric("IA", "Gemini 2.0" if st.session_state.is_premium else "Basique")
...
# ============== 4. ANALYSE IA PRO + BLOCAGE 5 ANALYSES ==============
    ...
    # Si clic depuis watchlist
    if st.session_state.selected_asset_from_watchlist:
        default_asset = st.session_state.selected_asset_from_watchlist
        st.session_state.selected_asset_from_watchlist = None
    else:
        default_asset = "Bitcoin"

    marché = st.selectbox("Marché", list(ASSETS.keys()))
    asset_name = st.selectbox("Actif", list(ASSETS[marché].keys()), index=list(ASSETS[marché].keys()).index(default_asset) if default_asset in ASSETS[marché].keys() else 0)
    ...
    if st.button("🚀 Lancer l'analyse"):
        ...
        if not st.session_state.is_premium:
            ...
            if st.session_state.analyses_count[today] >= 5:
                st.error("⚠️ Limite de 5 analyses gratuites atteinte. Passe Premium pour illimité.")
                st.stop()
        ...
        with st.spinner("Analyse en cours..."):
            ...
            if st.session_state.is_premium: st.subheader("🤖 Explication IA"); st.write(assistant_gpt4("Explique la tendance et donne 3 raisons du signal", f"Score {score}, RSI {rsi:.2f}, EMA20 {ema20:.2f}"))
            ...
            for marché, actifs in ASSETS.items():
                for name, tick in actifs.items():
                    ...
                    if not df.empty:
                        ...
                        prix = get_price(tick)
                        results.append({"Marché": marché, "Actif": name, "Prix": f"${prix:,.2f}", "Score": score, "Signal": signal, "RSI": f"{rsi:.2f}"})
            df_res = pd.DataFrame(results).sort_values(by="Score", ascending=False)
            st.dataframe(df_res, use_container_width=True)

#... LE RESTE DE TON CODE PORTEFEUILLE, BACKTEST, ETC RESTE IDENTIQUE...
