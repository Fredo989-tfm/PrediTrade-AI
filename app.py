...
# 📊 BACKTEST PREDITRADE AI
# ============================================================

elif menu=="📊 Backtest":

    st.title("📊 Backtest PrediTrade AI")
    st.write("Testez les performances de PrediTrade AI sur les données historiques.")
    ...
    with col2:
        actif_bt = st.selectbox(
            "Actif",
            ASSETS[type_actif_bt],
            key="bt_actif"
        )
    ...
    if st.button(
        ...
        try:
            ...
            with st.spinner(
                f"Analyse historique de {actif_bt}..."
            ):
                ...
                if df_bt.empty:
                  st.error("❌ Aucune donnée historique disponible.")
                  st.warning(
                  ...
                  st.write("Type d'actif :", type_actif_bt)
                    ...
                    st.warning(
                        ...
                        f"{len(df_bt)} bougies disponibles. "
                        f"Le moteur nécessite au moins 220 bougies."
                    )
                    ...
                    if resultat_bt.get("statut") != "OK":

                        st.warning(
                            resultat_bt.get(
                                "message",
                                "Le backtest n'a produit aucun résultat."
                            )
                        )
                        ...
                        capital_final = float(
                            resultat_bt["capital_final"]
                        )

                        profit_total = float(
                            resultat_bt["profit_total"]
                        )

                        rendement = float(
                            resultat_bt["rendement"]
                        )

                        nb_trades = int(
                            resultat_bt["nb_trades"]
                        )
                        ...
                        profit_factor = resultat_bt[
                            "profit_factor"
                        ]

                        max_drawdown = float(
                            resultat_bt["max_drawdown"]
                        )

                        score_robustesse = int(
                            resultat_bt["score_robustesse"]
                        )
                        ...
                        if profit_factor == float("inf"):
                            ...
                            pf_text = f"{float(profit_factor):.2f}"
                        ...
                        if (
                            ...
                            trades_temp = resultat_bt.get(
                                "trades"
                            )

                            if (
                                trades_temp is not None
                                and not trades_temp.empty
                            ):
                        ...
                        x1.metric(
                            "Gagnants",
                            resultat_bt["gagnants"]
                        )

                        x2.metric(
                            "Perdants",
                            resultat_bt["perdants"]
                        )

                        x3.metric(
                            "Meilleur trade",
                            f"${resultat_bt['meilleur_trade']:,.2f}"
                        )

                        x4.metric(
                            "Pire trade",
                            f"${resultat_bt['pire_trade']:,.2f}"
                        )
                        ...
                        x5.metric(
                            "Expectancy",
                            f"{resultat_bt['expectancy_R']:.2f} R"
                        )

                        x6.metric(
                            "Sharpe",
                            f"{resultat_bt['sharpe']:.2f}"
                        )
            ...
            st.error(
                "❌ Une erreur est survenue pendant le backtest."
            )
